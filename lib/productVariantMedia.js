...
const parseVariantKey = (variantKey) => {
  ...
  if (normalizedKey.startsWith('color:')) {
    return { sizeLabel: '', colorName: normalizedKey.slice(6) };
  }
  if (normalizedKey.startsWith('size:')) {
    return { sizeLabel: normalizedKey.slice(5), colorName: '' };
  }

  const [sizeLabel = '', colorName = ''] = normalizedKey.split('|');
  return { sizeLabel, colorName };
};

const getImageAssignments = (image) => (
  image.variantKeys?.length > 0
    ? image.variantKeys.map(parseVariantKey)
    : [{
        sizeLabel: String(image.sizeLabel || '').toLocaleLowerCase('en-IN'),
        colorName: String(image.colorName || '').toLocaleLowerCase('en-IN'),
      }]
);
...
const imageMatchesSelection = (image, selectedSize, selectedColor) => {
  if (image.variantKeys?.length > 0) {
    ...
    return getImageAssignments(image).some(({ sizeLabel, colorName }) => {
      if (normalizedColor && !normalizedSize) return colorName === normalizedColor;
      if (normalizedSize && !normalizedColor) return sizeLabel === normalizedSize;
      return (!sizeLabel || sizeLabel === normalizedSize)
        && (!colorName || colorName === normalizedColor);
    });
  }
...
export const getVariantGalleryImages = (images, selectedSize, selectedColor) => {
  ...
  let matchingImages = images.filter((image) => (
    imageMatchesSelection(image, selectedSize, selectedColor)
  ));

  if (normalizedColor) {
    const colorExclusiveImages = matchingImages.filter((image) => (
      getImageAssignments(image).every(({ colorName }) => !colorName || colorName === normalizedColor)
    ));
    if (colorExclusiveImages.length > 0) matchingImages = colorExclusiveImages;
  }
